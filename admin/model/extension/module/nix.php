...
class ModelExtensionModuleNix extends Model {
	public function install() {
      ...
      PRIMARY KEY (`supplier_id`)
		...
		// Add nix_supplier_id to identity imported entities by supplier
	...
	public function addCategory($data) {
		...
		foreach ($data['category_description'] as $language_id => $value) {
			...
			if (isset($value['meta_h1'])) $sql .= ", meta_h1 = '" . $this->db->escape($value['meta_h1']) . "'";			
...
//			. " AND p.sku = '" . $this->db->escape($filter['sku']) . "'"
	...
	public function addProduct($data) {
		...
		if (isset($data['keyword'])) {
			$data['keyword'] = $product_id . '-' . $data['keyword'];
	...
	public function getManufacturer($name) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "manufacturer` WHERE name = '" . $name . "'");
		return $query->row;
	}
	...
	public function addManufacturer($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "manufacturer_description SET manufacturer_id = '" . (int)$manufacturer_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($data['name']) . "', description = '" . $this->db->escape($data['description']) . "', meta_title = '" . $this->db->escape($data['name']) . "', meta_h1 = '" . $this->db->escape($data['name']) . "', meta_description = '" . $this->db->escape($data['description']) . "', meta_keyword = '" . $this->db->escape($data['meta_keyword']) . "'");
