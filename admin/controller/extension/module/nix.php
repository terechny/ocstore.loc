...
//		$data['primary_language'] = $this->request->post['primary_language'] ?? 0;
	...
	public function processingImportAjax() {
		...
		if (isset($this->request->post['nix_new_submit'])) {
			foreach ($this->languages as $language) {
				if ($_FILES['xmlfile']['tmp_name'][$language['language_id']]) {
					$this->stdelog->write(4, 'tmp-' . $language['language_id'] . '.xml', 'processingImportAjax() :: try to record file');
					file_put_contents('tmp-' . $language['language_id'] . '.xml', file_get_contents($_FILES['xmlfile']['tmp_name'][$language['language_id']]));
				}
			}
		}	else {
			if (!is_file('tmp-' . $this->request->post['language_id'] . '.xml')) {
		...
		foreach ($this->languages as $language) {
			if (is_file('tmp-' . $language['language_id'] . '.xml')) {
				$use_errors = libxml_use_internal_errors(true);			
				
				$this->xml[$language['language_id']] = simplexml_load_file('tmp-' . $language['language_id'] . '.xml');
		...
		if (false === $this->xml[$this->request->post['language_id']]) {
			$this->errors['warning'] = $this->language->get('error_import_fatal');
		...
		if (isset($main_xml->shop->offers)) {
			//$result = $this->recordProduct($main_xml->shop->offers);
			$result = $this->recordProduct($main_xml->xpath('shop/offers/offer'));
	...
	protected function cleanUp() {
		...
		foreach ($this->languages as $language) {
			if (is_file('tmp-' . $language['language_id'] . '.xml')) {
				unlink('tmp-' . $language['language_id'] . '.xml');
			}
		}
	}
...
//		}
	...
	private function recordProduct($offers) {
		...
		$tag_name							 = $this->supplier['tags']['name'];
		$tag_price						 = $this->supplier['tags']['price'];
		...
		foreach ($offers as $node_index => $offer) {
			$this->stdelog->write(2, (string)$offer['id'], 'recordProduct() :: NEW ITTERATION $offer with $offer["id"]');
...
//					'sku' => trim($offer->$tag_sku),
			...
			if ($test_product == [] 
				...
				&& !isset($offer->$offer->$tag_attributes) 
			...
			if (isset($offer->$tag_price)) {
				$price = $offer->$tag_price;
			}			
			...
			if (isset($offer->$tag_attributes)) {
				...
				$product_attribute = $this->prepareAttributes($node_index);
			}
			...
			if ($test_product == []) {
				...
				$product_description = $this->prepareProductDescription($node_index);
				...
				$data = [
					...
					// 
					'keyword'	=> $this->helperTranslitUniversal($product_description[$this->request->post['language_id']]['name']),
	...
	private function updateProductStock($offer, $test_product, $filter) {
		$this->stdelog->write(4, 'updateProductStock() is called');
	...
	private function prepareProductDescription($node_index) {
		$this->stdelog->write(4, $node_index, 'getProductDescription() called with');
		...
		foreach ($this->xml as $language_id => $xml) {
			$this_lang_node = $xml->shop->offers->offer[$node_index];
			...
			if (isset($this_lang_node->$tag_name)) {
				$product_description[$language_id]['name'] = trim((string)$this_lang_node->$tag_name);
			}
			
			if (isset($this_lang_node->$tag_description)) {
				$product_description[$language_id]['description'] = trim((string)$this_lang_node->$tag_description);
			}
		}
	...
	private function prepareAttributes($node_index) {
		$product_attribute = [];
	
		$languages_used = [];
		
		foreach ($this->xml as $language_id => $xml) {
			${'params_' . $language_id} = $xml->shop->offers->offer[$node_index]->{$this->supplier['tags']['attributes']};
		...
		// Attribute Group
		$test_attribute_group = $this->model_extension_module_nix->getAttributeGroup('Default');
		...
		if ($test_attribute_group == []) {
			...
			foreach ($languages_used as $language_id => $dummy_value) {
				$attribute_group_data['attribute_group_description'][$language_id]['name'] = 'Default';
			}
		...
		$this->stdelog->write(4, ${'params_' . $this->request->post['language_id']}, 'prepareAttributes() :: ${"params_" . $this->request->post["language_id"]}');
		...
		foreach ($this->xml[$this->request->post['language_id']]->shop->offers->offer[$node_index]->{$this->supplier['tags']['attributes']} as $attribute) {
			...
			if ($test_attribute !== []) {
				...
				foreach ($languages_used as $language_id => $dummy_value) {
					...
					$this->stdelog->write(4, $param_item, 'prepareAttributes() :: $param_item for language_id - ' . $language_id);
					
					$dta['attribute_description'][$language_id]['name'] = (string)$param_item[$i]['name'];
				}
				
				$this->stdelog->write(4, $dta['attribute_description'], 'prepareAttributes() :: $dta["attribute_description"] before $this->model_extension_module_nix->addAttribute($dta)');
