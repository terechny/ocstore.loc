...
class Googleshopping extends Library {
    ...
    public function __construct($registry, $store_id) {
        ...
        if ($this->store_id === 0) {
            ...
            $this->store_url = $this->model_setting_setting->getSetting('config_ssl', $store_id);
            $this->store_name = $this->model_setting_setting->getSetting('config_name', $store_id);
        }
    ...
    public function cron() {
        ...
        foreach ($this->getJobs() as $job) {
            try {
                ...
                $this->config->set("config_seo_url", $this->model_setting_setting->getSetting("config_seo_url", $this->store_id));
    ...
    public function getCampaignReports() {
        ...
        $cache = new \Cache($this->config->get('cache_type'), self::CACHE_CAMPAIGN_REPORT);
            ...
            if (is_array($rows) && $rows) {
                foreach ($rows as $key => $row) {
                    ...
                    foreach ($row as $k => $item_value) {
                        if (array_key_exists($k, $value_keys) && array_key_exists($item_value, $campaign_keys) && $value_keys[$k] == 'campaign_name') {
                            $l = $campaign_keys[$item_value];
                        }
                    }
    ...
    public function getProductReports($product_ids) {
        $cache = new \Cache($this->config->get('cache_type'), self::CACHE_PRODUCT_REPORT);
